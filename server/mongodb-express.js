...
app.get('/movies/director', function(req, res) {
    ...
    MongoClient.connect(uri, function(err, db) {
        if (err) throw err;
        var dbo = db.db("imdb");
        dbo.collection("movies").find({ "name": film }, { projection: { "_id": 0, "name": 1, "year": 1, "directors.first_name": 1, "directors.last_name": 1 } }).toArray(function(err, result) {
            if (err) throw err;
            res.send(result);
            db.close();
        });
...
app.get('/movies/genre', function(req, res) {
    ...
    MongoClient.connect(uri, function(err, db) {
        if (err) throw err;
        var dbo = db.db("imdb");
        dbo.collection("movies").aggregate({ $unwind: { path: "$genres" } }, { $match: { $and: [{ genres: genre }, { rank: { $ne: null } }] } }, { $sort: { rank: -1 } }, { $limit: 10 }, { $project: { _id: 1, name: 1, year: 1, rank: 1, directors: { id: 1, first_name: 1, last_name: 1 } } })
...
app.get('/actors/top10/year', function(req, res) {
    ...
    MongoClient.connect(uri, function(err, db) {
        if (err) throw err;
        var dbo = db.db("imdb");
        dbo.collection("movies").aggregate({ $match: { "year": year } }, { $unwind: { path: '$actors' } }, { $group: { _id: "$actors.actor_id", count: { $sum: 1 } } }, { $sort: { "count": -1 } }, { $limit: 10 })
            .toArray(function(err, result) {
                if (err) throw err;
...
app.get('/movies/bestmovie/actor', function(req, res) {
    ...
    MongoClient.connect(uri, function(err, db) {
        if (err) throw err;
        var dbo = db.db("imdb");
        dbo.collection("actors").find({ "last_name": lastName, "first_name": firstName }, { projection: { "_id": 1 } }).toArray(function(err, result) {
            if (err) throw err;
            console.log(result);
...
app.get('/movies/evolution/year', function(req, res) {
    ...
    MongoClient.connect(uri, function(err, db) {
        ...
        dbo.collection("movies").aggregate([
                ...
                { $group: { "_id": "$_id.year", "types": { $push: { "genre": "$_id.genres", total: "$total" } }, "grandTotal": { $sum: "$total" } } },
                { $unwind: "$types" },
                { $project: { "_id": 0, "genre": "$types.genre", "percentage": { $divide: ["$types.total", "$grandTotal"] }, "year": "$_id" } },
...
app.get('/movies/director/evolution', function(req, res) {
    MongoClient.connect(uri, function(err, db) {
        if (err) throw err;
        var dbo = db.db("imdb");
        dbo.collection("movies").aggregate({ $match: { "rank": { "$ne": null } } }, { $unwind: { path: "$directors" } }, { $project: { "year": 1, "rank": 1, "genres": 1, "directors": { "id": 1, "first_name": 1, "last_name": 1 } } }, { $unwind: { path: "$genres" } }, {
                $group: {
...
app.get('/movies/genre/bestactor', function(req, res) {
    ...
    MongoClient.connect(uri, function(err, db) {
        ...
        dbo.collection("movies").aggregate(
            ...
            .toArray(function(err, result) {
                if (err) throw err;
                dbo.collection("actors").find({ "_id": result[0]["_id"] }, { projection: { "last_name": 1, "first_name": 1, "_id": 0 } }).toArray(function(err, resultFinal) {
                    if (err) throw err;
                    res.send({
...
app.get('/actors/genre', function(req, res) {
    ...
    MongoClient.connect(uri, function(err, db) {
        ...
        dbo.collection("actors").find({ "last_name": lastName, "first_name": firstName }, { projection: { "_id": 1 } })
            .toArray(function(err, result) {
                if (err) throw err;
                dbo.collection("movies").aggregate({ $unwind: { path: "$actors" } }, { $match: { "actors.actor_id": result[0]["_id"] } }, { $unwind: { path: "$genres" } }, { $facet: { totalCount: [{ $count: "value" }], pipelineResults: [{ $project: { _id: 1, genres: 1 } }] } }, { $unwind: { path: "$pipelineResults" } }, { $unwind: { path: "$totalCount" } }, { $replaceRoot: { newRoot: { $mergeObjects: ["$pipelineResults", { totalCount: "$totalCount.value" }] } } }, { $group: { _id: "$genres", count: { $sum: 1 }, totalCount: { $first: "$totalCount" } } }, { $project: { "count": 1, "totalCount": 1, "percentage": { "$multiply": [{ "$divide": [100, "$totalCount"] }, "$count"] } } })

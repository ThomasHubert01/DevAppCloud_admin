...
app.get('/movies/director', function (req, res) {
    ...
    MongoClient.connect(uri, function (err, db) {
        if (err) throw err;
        var dbo = db.db("imdb");
        dbo.collection("movies").find(
            { "name": film },
            { projection: { "_id": 0, "name": 1, "year": 1, "directors.first_name": 1, "directors.last_name": 1 } }).toArray(function (err, result) {
                if (err) throw err;
                res.send(result);
                db.close();
            });
...
app.get('/movies/genre', function (req, res) {
    ...
    MongoClient.connect(uri, function (err, db) {
        ...
        dbo.collection("movies").aggregate(
            { $unwind: { path: "$genres" } },
            { $match: { $and: [{ genres: genre }, { rank: { $ne: null } }] } },
            { $sort: { rank: -1 } },
            { $limit: 10 },
            { $project: { _id: 1, name: 1, year: 1, rank: 1, directors: { id: 1, first_name: 1, last_name: 1 } } })
...
app.get('/actors/top10/year', function (req, res) {
    ...
    MongoClient.connect(uri, function (err, db) {
        ...
        dbo.collection("movies").aggregate(
            { $match: { "year": year } },
            { $unwind: { path: '$actors' } },
            { $group: { _id: "$actors.actor_id", count: { $sum: 1 } } },
            { $sort: { "count": -1 } },
            { $limit: 10 })
            .toArray(function (err, result) {
                if (err) throw err;
...
app.get('/movies/bestmovie/actor', function (req, res) {
    ...
    MongoClient.connect(uri, function (err, db) {
        ...
        dbo.collection("actors").find(
            ...
            if (err) throw err;
            dbo.collection("movies").find(
                { "actors.actor_id": result[0]["_id"] }, { projection: { "actors": 0 } })
...
app.get('/movies/director/evolution', function (req, res) {
    MongoClient.connect(uri, function (err, db) {
        ...
        dbo.collection("movies").aggregate(
            ...
            { $project: { "year": 1, "rank": 1, "genres": 1, "directors": { "id": 1, "first_name": 1, "last_name": 1 } } },
            { $unwind: { path: "$genres" } },
            {
                $group:
                {
                    ...
                    last_name: { "$first": "$directors.last_name" }, success: { "$avg": "$rank" }
                }
            },
            {
                $group: {
    ...
    MongoClient.connect(uri, function (err, db) {
        ...
        dbo.collection("movies").aggregate(
            ...
            .toArray(function (err, result) {
                if (err) throw err;
                dbo.collection("actors").find(
                    { "_id": result[0]["_id"] }, { projection: { "last_name": 1, "first_name": 1, "_id": 0 } }).toArray(function (err, resultFinal) {
                        if (err) throw err;
                        res.send({
    ...
    MongoClient.connect(uri, function (err, db) {
        ...
        dbo.collection("actors").find(
            ...
            .toArray(function (err, result) {
                ...
                dbo.collection("movies").aggregate(
                    { $unwind: { path: "$actors" } },
                    { $match: { "actors.actor_id": result[0]["_id"] } },
                    { $unwind: { path: "$genres" } },
                    { $facet: { totalCount: [{ $count: "value" }], pipelineResults: [{ $project: { _id: 1, genres: 1 } }] } },
                    ...
                    { $group: { _id: "$genres", count: { $sum: 1 }, totalCount: { $first: "$totalCount" } } },
                    { $project: { "count": 1, "totalCount": 1, "percentage": { "$multiply": [{ "$divide": [100, "$totalCount"] }, "$count"] } } })
